...
const IngredientCreateModal = ({ show, onHide, onIngredientCreate }) => {
...
const IngredientCreateModal = ({ show, onHide, onIngredientCreate }) => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/admin/ingredients`, submitFormData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      onIngredientCreate(res.data);
      handleHide();
    } catch (err) {
      const errorMsg = err.response?.data?.errors?.[0]?.msg || err.response?.data?.msg || t('create_ingredient_failed');
      setError(errorMsg);
      console.error(err);
    }
  };
    ...
    <Modal show={show} onHide={handleHide} centered size="lg">
      <Form onSubmit={onSubmit}>
        <Modal.Header closeButton>
          <Modal.Title>{t('create_ingredient')}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {error && <Alert variant="danger">{error}</Alert>}
          
          {loading ? <p>{t('loading_dependencies')}...</p> : (
            <>
...
export default IngredientCreateModal;
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/admin/ingredients`, formData);
      onIngredientCreate(res.data);
      handleHide();
    } catch (err) {
      const errorMsg = err.response?.data?.errors?.[0]?.msg || err.response?.data?.msg || t('create_ingredient_failed');
        ...
        <Modal.Body>
          ...
          {loading ? <p>{t('loading_dependencies')}...</p> : (
            <>
              ...
              <Form.Group className="mb-3" controlId="createFormImage">
