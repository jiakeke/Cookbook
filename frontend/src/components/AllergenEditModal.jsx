...
const AllergenEditModal = ({ show, onHide, allergen, onAllergenUpdate }) => {
  ...
  useEffect(() => {
    if (allergen) {
      setFormData({
        name: allergen.name || { en: '', fi: '', zh: '' },
        description: allergen.description || { en: '', fi: '', zh: '' },
      });
      setError('');
      setActiveLang('en');
    }
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/admin/allergens/${allergen._id}`, formData);
      onAllergenUpdate(res.data);
      onHide();
    } catch (err) {
      const errorMsg = err.response?.data?.errors?.[0]?.msg || err.response?.data?.msg || t('update_allergen_failed');
      setError(errorMsg);
      console.error(err);
    }
  };
    ...
    <Modal show={show} onHide={handleHide} centered>
      <Form onSubmit={onSubmit}>
        <Modal.Header closeButton>
          <Modal.Title>{t('edit_allergen')}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
