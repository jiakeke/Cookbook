...
const SpecialGroupCreateModal = ({ show, onHide, onSpecialGroupCreate }) => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/admin/specialgroups`, formData);
      onSpecialGroupCreate(res.data);
      handleHide();
    } catch (err) {
      const errorMsg = err.response?.data?.errors?.[0]?.msg || err.response?.data?.msg || t('create_special_group_failed');
      setError(errorMsg);
      console.error(err);
    }
  };
    ...
    <Modal show={show} onHide={handleHide} centered>
      <Form onSubmit={onSubmit}>
        <Modal.Header closeButton>
          <Modal.Title>{t('create_special_group')}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
